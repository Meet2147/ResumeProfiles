...
from langchain_huggingface import uuid
import os
from fastapi import HTTPException, UploadFile
from langchain.text_splitter import RecursiveCharacterTextSplitter
from app.config.weaviate import connect_to_weaviate
...
def create_pdf(content: str, filename: str) -> str:
    """
    Creates a PDF from the given Markdown content.
...
async def update_skills(user_id: int, new_skills: str):
    try:
        ...
        create_pdf(updated_content, pdf_filename)
        print(f"PDF created: {pdf_filename}")

        # Get Weaviate client
        client = await connect_to_weaviate()

        # Handle the 'chunks' field safely
        chunks = resume.get("chunks", [])
        if not chunks:
            print(f"No chunks found for resume with user_id {user_id}. Skipping Weaviate update.")
        else:
            # Update embeddings in Weaviate for each chunk
            class_name = "Resume"
        ...
        return {
            "message": f"Skills updated successfully for user ID {user_id}.",
            "pdf": pdf_filename
        }
...
async def update_certifications(user_id: int, new_certifications: str):
    try:
        # Get MongoDB collection
        collection = get_resume_collection()
        print("New Certification", new_certifications)
        ...
        return {
            "message": f"Certifications updated successfully for user ID {user_id}.",
            "pdf": pdf_filename
        }
...
async def update_projects(user_id: int, new_projects: str):
    try:
        ...
        return {
            "message": f"Projects updated successfully for user ID {user_id}.",
            "pdf": pdf_filename
        }
